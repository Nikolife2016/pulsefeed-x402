...
export class SsrfBlocked extends Error {
    constructor(reason) { super(`blocked: ${reason}`); this.name = "SsrfBlocked"; }
}
function ipv4Blocked(ip) {
    const p = ip.split(".").map(Number);
    if (p.length !== 4 || p.some(n => !Number.isInteger(n) || n < 0 || n > 255))
        return "невалидный IPv4";
    const [a, b] = p;
    if (a === 0)
        return "0.0.0.0/8";
    if (a === 10)
        return "10/8 (частная сеть)";
    if (a === 127)
        return "127/8 (loopback)";
    if (a === 169 && b === 254)
        return "169.254/16 (link-local / cloud metadata)";
    if (a === 172 && b >= 16 && b <= 31)
        return "172.16/12 (частная сеть)";
    if (a === 192 && b === 168)
        return "192.168/16 (частная сеть)";
    if (a === 100 && b >= 64 && b <= 127)
        return "100.64/10 (CGNAT)";
    if (a === 192 && b === 0)
        return "192.0/16 (спец. назначение)";
    if (a >= 224)
        return ">=224/4 (multicast/reserved)";
    return null;
}
function ipv6Blocked(ip) {
    const s = ip.toLowerCase().replace(/^\[|\]$/g, "");
    if (s === "::1" || s === "::")
        return "IPv6 loopback/unspecified";
    ...
    if (m)
        return ipv4Blocked(m[1]);
    return null;
}
export function ipBlockedReason(ip) {
...
export async function assertSafeUrl(raw) {
    ...
    try {
        ...
        throw new SsrfBlocked("невалидный URL");
    }
    if (u.protocol !== "http:" && u.protocol !== "https:")
        throw new SsrfBlocked(`схема ${u.protocol}`);
    const host = u.hostname.replace(/^\[|\]$/g, "");
    // Литеральный IP — проверяем сразу, без DNS.
    if (net.isIP(host)) {
        const r = ipBlockedReason(host);
        if (r)
            throw new SsrfBlocked(r);
        return u;
    }
    ...
    try {
        ...
        throw new SsrfBlocked(`DNS не резолвится: ${host}`);
    }
    if (!addrs.length)
        throw new SsrfBlocked(`нет A/AAAA: ${host}`);
    for (const a of addrs) {
        const r = ipBlockedReason(a.address);
        if (r)
            throw new SsrfBlocked(`${host} → ${a.address}: ${r}`);
    }
    return u;
}
