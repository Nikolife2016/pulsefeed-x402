...
- New export `PulseFeedUnavailableError`: `verify()` throws it when PulseFeed answers with a non-2xx
  status, non-JSON, a body that is not a verdict, or times out. **Minor.**
...
- Previous release from the service repository (x402 v1 `x402-fetch` example).
